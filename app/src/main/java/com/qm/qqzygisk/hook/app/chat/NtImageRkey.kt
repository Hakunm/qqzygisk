...
internal object NtImageRkey {
    ...
    fun parseOrScan(buffer: ByteArray, nowMillis: Long): RkeySnapshot =
        runCatching { parse(buffer, nowMillis) }.getOrElse { scan(buffer, nowMillis) }

    fun scan(buffer: ByteArray, nowMillis: Long): RkeySnapshot {
        val values = RKEY_VALUE.findAll(buffer.toString(Charsets.ISO_8859_1))
            .map { it.value }
            .distinct()
            .toList()
        check(values.isNotEmpty()) { "缓冲区里没有 NT 图片 rkey" }
        val byType = linkedMapOf<Int, String>()
        values.forEachIndexed { index, value ->
            ...
            byType[type] = if (value.startsWith("rkey=")) "&$value" else value
        }
        return RkeySnapshot(byType, nowMillis + DEFAULT_TTL_MS)
    }

    fun isRkeyCommand(command: String?): Boolean {
        val cmd = command.orEmpty()
        return cmd == SERVICE_COMMAND ||
            cmd.contains("0x9067", ignoreCase = true) ||
            cmd.contains("9067_202")
    }

    fun containsRkey(buffer: ByteArray?): Boolean {
        if (buffer == null || buffer.size < 8) return false
        val text = buffer.toString(Charsets.ISO_8859_1)
        return text.contains("rkey=")
    }

    fun signedCandidates(url: String, snapshot: RkeySnapshot?): List<String> {
        val out = linkedSetOf<String>()
        if (url.contains("rkey=")) out += url
        snapshot?.let { keys ->
            select(url, keys)?.let { out += apply(url, it) }
            keys.byType.values.forEach { value ->
                if (value.isNotBlank()) out += apply(url, value)
            }
        }
        out += url
        return out.toList()
    ...
    const val SERVICE_COMMAND = "OidbSvcTrpcTcp.0x9067_202"

    private val RKEY_VALUE = Regex("""rkey=[^&\u0000\s]+""")
